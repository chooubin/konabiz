    ...
    methods: {
        ...
        doGetProductList: async function () {
            ...
            if (code === 1) {
                ...
                if (!entity.length) {
                    html = '<option value="">상품을 선택해주세요</option>'
                }
            } else {
                switch (code) {
                    // 예외처리 경우
        ...
        doGetWelfareUsageList: async function (type = 1) {
            ...
            if (_this.params.searchType === "6") {
             ...
             if (type === -1) {
                 ...
                 params.limit = -1;
                 const res = await ServiceExec.post("/api/payment/failure/doGetWelfarePaymentFailureList", _this.params);
                 const entity = res.entity;
                 if (entity.totalCount === 0) {
                     alert("다운로드 할 데이터가 없습니다.");
                     return;
                 }
                 DOWNLOAD_MODAL.methods.download("/api/payment/failure/doGetWelfarePaymentFailureList", params);
                 return;
             }
