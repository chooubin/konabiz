...
const FH = {
    ...
    virtualNum: 0,
    getList: function () {
        _this.methods.doGetCorporatePointPaymentFailureList();
    },
    events: {
        /**
         * key 이벤트
    ...
    methods: {
         ...
         * @returns {Promise<void>}
         */
        doGetCorporatePointPaymentFailureList: async function (type = 1) {
            const searchStartDate = $("#searchStartDate").val();
            ...
            if (type === -1) {
                 ...
                 params.limit = -1;
                 const res = await ServiceExec.post("/api/payment/failure/doGetCorporatePointPaymentFailureList", _this.params);
                 const entity = res.entity;
                 if (entity.totalCount === 0) {
                     alert("다운로드 할 데이터가 없습니다.");
                     return;
                 }
                 DOWNLOAD_MODAL.methods.download("/api/payment/failure/doGetCorporatePointPaymentFailureList", params);
                 return;
             }
