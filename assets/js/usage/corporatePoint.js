...
const FH = {
    ...
    virtualNum: 0,
    getList: function () {
        _this.methods.doGetCorporatePointUsageList();
    },
    events: {
        /**
         * key 이벤트
    ...
    methods: {
        ...
        setTable: function () {
            ...
            const lsn = userLoginId+'_usageCorporatePointGridColumns';
        ...
        /**
         * 법인카드(포인트) 이용 내역 리스트 - 데이터 조회
         * @returns {Promise<void>}
         */
        doGetCorporatePointUsageList: async function (type = 1) {
            const searchStartDate = $("#searchStartDate").val();
            ...
            if (type === -1) {
                 ...
                 params.limit = -1;
                 const res = await ServiceExec.post("/api/usage/doGetCorporatePointUsageList", _this.params);
                 const entity = res.entity;
                 if (entity.totalCount === 0) {
                     alert("다운로드 할 데이터가 없습니다.");
                     return;
                 }
                 DOWNLOAD_MODAL.methods.download("/api/usage/doGetCorporatePointUsageList", params);
                 return;
             }
        ...
        downloadPdf: async function (crdType) {
            ...
            const pdfDataList = validRows.map(item => ({
                transactionAmount: item.pointAmt,          // 포인트 사용금액
                cardType: item.svcNm,                                        // 상품명
                cardNumber: item.cdno,                                       // 카드번호 (마스킹됨)
                transactionDateTime: item.tranDt,                             // 거래일시
                transactionType: item.tranSubTypeNm,                          // 거래유형 / 거래타입
                approvalNumber: item.apvNo || '',                             // 승인번호
                supplyPrice: null,                                           // 공급가액 (데이터 없음)
                surtax: null,                                                // 부가세 (데이터 없음)
                serviceCharge: null,                                         // 봉사료 (데이터 없음)
                totalTransactionAmount: item.pointAmt,     // 총 거래 금액 (포인트 사용금액)
