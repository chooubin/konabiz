...
const FH = {
    ...
    virtualNum: 0,
    getList: function () {
        _this.methods.doGetCorporateUsageList();
    },
    events: {
        /**
         * key 이벤트
    ...
    methods: {
        ...
        setTable: function () {
            ...
            class CheckboxRenderer {
                ...
                render(props) {
                    ...
                    const tranSubTypeNm = grid.getValue(rowKey, "prcsNm") || '';
         ...
         * @returns {Promise<void>}
         */
        doGetCorporateUsageList: async function (type = 1) {
            const searchStartDate = $("#searchStartDate").val();
            ...
            if( _this.params.searchType === "6" ) {
             ...
             if (type === -1) {
                 ...
                 params.limit = -1;
                 const res = await ServiceExec.post("/api/usage/doGetCorporateUsageList", _this.params);
                 const entity = res.entity;
                 if (entity.totalCount === 0) {
                     alert("다운로드 할 데이터가 없습니다.");
                     return;
                 }
                 DOWNLOAD_MODAL.methods.download("/api/usage/doGetCorporateUsageList", params);
                 return;
             }
        ...
        downloadPdf: async function (crdType) {
            ...
            const validRows = checkedRows.filter(row => {
                const tranSubTypeNm = row.prcsNm || '';
            ...
            const pdfDataList = validRows.map(item => ({
                transactionAmount: item.trAmt,         // 포인트 사용금액
                cardType: item.svcNm,                                        // 상품명
                cardNumber: item.cdno,                                       // 카드번호 (마스킹됨)
                transactionDateTime: item.iasTranDt,                         // 거래일시 (DTO field)
                transactionType: item.prcsNm,              // 거래유형 (prcsNm 또는 orgNrNm)
                approvalNumber: item.apvNo || '',                            // 승인번호
                supplyPrice: item.supplyAmt|| '',              // 공급가액
                surtax: item.vat|| '',                          // 부가세
                serviceCharge: item.depositCupAmt|| '',         // 봉사료 / 1회용컵 보증금
                totalTransactionAmount: item.trAmt,     // 총 거래 금액 (포인트 사용금액)
                affiliateStoreName: item.mctNm || '',                        // 가맹점명
                industry: item.bztpNm || '',                                 // 업종
                affiliateNumber: item.mctId || '',                            // 가맹점번호
                affiliateStoreAddress: '',                                    // 가맹점주소 (데이터 없음)
                businessRegistrationNumber: item.bizLicenseNo || '',          // 사업자번호
                apvDt: item.apvDt || '',
                nrNo: item.nrNo || '',
                par: item.par || ''
            }));
