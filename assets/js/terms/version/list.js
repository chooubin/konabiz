...
const FH = {
    ...
    methods: {
         ...
         */
        termsForm: function (type = "reg", prvTypeCd) {
            if (type == "reg") {
                // 마지막 버전에서 0.1 버전 증가 -> 등록 페이지에서 파라미터로 받은 후, 노출만 시키는 용도
                let wptlPrvAgrmVerNo = (Number(_this.wptlPrvAgrmVerNo) + 0.1).toFixed(1);
                Util.href("/terms/version/form", {wptlPrvNo: 0, wptlPrvAgrmVerNo: wptlPrvAgrmVerNo, prvTypeCd: prvTypeCd});
            } else {
                Util.href("/terms/version/form", {wptlPrvNo: _this.wptlPrvNo, prvTypeCd: prvTypeCd});
            }
        }
    },
    init: function () {
