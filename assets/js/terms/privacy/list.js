...
const FH = {
    ...
    methods: {
        ...
        /**
         * 개인정보 취급 방침 - 등록/수정 페이지로 이동
         * @param type (등록: reg, 수정: mod)
         */
        termsForm: function (type = "reg") {
            if (type == "reg") {
                // 지막 버전에서 0.1 버전 증가 -> 등록 페이지에서 파라미터로 받은 후, 노출만 시키는 용도
                let wptlPrvAgrmVerNo = (Number(_this.wptlPrvAgrmVerNo) + 0.1).toFixed(1);
                Util.href("/terms/privacy/form", {wptlPrvNo: 0, wptlPrvAgrmVerNo: wptlPrvAgrmVerNo})
            } else {
                Util.href("/terms/privacy/form", {wptlPrvNo: _this.wptlPrvNo})
            }
        }
    },
    init: function () {
